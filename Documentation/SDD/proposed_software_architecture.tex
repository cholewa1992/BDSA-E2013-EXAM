...
The global control flow is designed with the use of threads to support concurrent users. The WebServer listen for new requests from the web client. When the WebServer receives a request it start a new thread that create a new RequestDelegater who process the request to Storage. If any, the Webserver receives the answer and send it to the webclient through the Communication Framework
