...
namespace FlatFileStorage
{
    ...
    public class InMemoryStorageConnection : IStorageConnection
    {
        ...
        /// <returns></returns>
        private InMemoryStorage<TEntity> GetSet<TEntity>() where TEntity : class, IEntityDto
        {
            if (!_sets.ContainsKey(typeof (TEntity)))
            {
                _sets[typeof (TEntity)] = new InMemoryStorage<TEntity>();
            } 
            return (InMemoryStorage<TEntity>) _sets[typeof(TEntity)];
        }
